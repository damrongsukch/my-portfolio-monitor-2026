...
function assetKind(ticker) { return /SPMO|QQQI|IAUI/i.test(ticker) ? "ETF" : "หุ้นสหรัฐ"; }
...
function renderHoldings(filter = activeFilter, query = document.getElementById("holdingSearch")?.value || "") {
  ...
  setHtml("mobileHoldings", rows.map(item => { const plClass = String(item.pl).startsWith("-") ? "negative" : item.pl === "-" ? "neutral" : "positive"; const layer = layerClass(item.layer); const gain = signedCurrencyFromThb(holdingGainThb(item)); return `<article class="mobile-holding-card ${plClass}"><div class="mobile-holding-top"><span class="ticker-cell">${tickerLogo(item.ticker)}<span><strong>${item.ticker}</strong><br><small>${assetKind(item.ticker)} <b>${Number(item.weight || 0).toFixed(1)}%</b> · <span class="layer-text ${layer}">${layer.toUpperCase()}</span></small></span></span>${signalBadge(item.signal)}</div><div class="holding-metrics"><div><span>จำนวน</span><strong>${numberFrom(item.shares).toFixed(7)}</strong></div><div><span>ต้นทุนเฉลี่ย</span><strong>${formatCurrencyFromUsd(item.avgCostUsd)}</strong></div><div><span>ราคาปัจจุบัน</span><strong>${formatCurrencyFromUsd(item.currentPriceUsd || item.price)}</strong></div><div><span>มูลค่า</span><strong>${formatCurrencyFromThb(item.value)}</strong></div></div><div class="mobile-holding-meta"><span>กำไร/ขาดทุน</span><strong class="${plClass}">${gain}<small>${item.pl}</small></strong></div></article>`; }).join("") || `<div class="empty">No holdings match this filter.</div>`);
}
...
function dcaReason(item, multiplier) { const signal = cleanSignal(item.signal); const rsi7 = numberFrom(item.rsi7), rsi14 = numberFrom(item.rsi14); if (!multiplier) return `ข้าม: ${signal}; RSI ${rsi7.toFixed(1)}/${rsi14.toFixed(1)} ยังไม่ใช่จังหวะซื้อที่ชัด`; if (multiplier === 1) return "1.0x: RSI ย่อตัวหรือ BUY DIP เป็นโซนที่น่าสะสมที่สุดวันนี้"; if (multiplier === 0.75) return "0.75x: สัญญาณสะสมดี แต่ยังไม่ใช่ไม้เต็ม"; if (multiplier === 0.5) return "0.5x: เทรนด์ยังดี ใช้ไม้กลางเพื่อลดการไล่ราคา"; return "0.25x: สัญญาณอ่อนกว่า ใช้ไม้เล็กเท่านั้น"; }
...
function renderSmartDca() { const input = document.getElementById("dcaBudgetInput"); const budget = parseBudgetInput(input?.value || ""); const plan = buildDcaPlan(budget.usd); const rows = budget.usd > 0 ? plan.picks.filter(item => item.amountUsd > 0) : plan.picks; setHtml("dcaBudgetSummary", budget.usd > 0 ? `<span class="dca-summary-title">แผนวันนี้: ใช้ ${formatUsd(plan.usedUsd)} จากงบ ${formatUsd(budget.usd)} และกันเงินสดไว้ ${formatUsd(plan.leftoverUsd)}</span><span class="dca-figures"><b>Budget ${formatUsd(budget.usd)}</b><b>Use ${formatUsd(plan.usedUsd)}</b><b>Cash left ${formatUsd(plan.leftoverUsd)}</b><b>Min ${formatUsd(MIN_ORDER_USD)}</b></span>` : "ใส่งบเป็น USD เท่านั้น เช่น 10 หรือ $10 แล้วระบบจะจัดอันดับซื้อจาก Signal + RSI"); setHtml("smartDcaList", rows.map((item, index) => `<div class="mini-row dca-plan-row"><span>${index + 1}. <strong>${item.ticker}</strong><small>${item.multiplier}x - RSI ${numberFrom(item.rsi7).toFixed(1)}/${numberFrom(item.rsi14).toFixed(1)} - ${item.reason}${item.belowMin ? " - below DIME minimum" : ""}</small></span><strong>${budget.usd > 0 ? formatUsd(item.amountUsd) : `${item.multiplier}x`}</strong></div>`).join("") || `<div class="empty">วันนี้ยังไม่มีจังหวะซื้อที่ชัดพอ เก็บเงินสดไว้ก่อน</div>`); const best = rows[0]; if (best) { setText("todaySignal", budget.usd > 0 ? "Sizing Ready" : "Signal Ready"); setText("todaySignalText", `${best.ticker}: ${best.multiplier}x เพราะ ${best.reason}. ใช้เป็นตัวช่วยจัดขนาดไม้ก่อนซื้อจริง`); } }
...
function renderAlerts() { const rows = []; holdings.forEach(item => { const r = numberFrom(item.pl); const signal = cleanSignal(item.signal); if (/strong buy|buy|accumulate/i.test(signal)) rows.push({ title: `${item.ticker} มีสัญญาณเข้าไม้`, text: `${signal} จาก Looker signal sheet. เช็กตลาดจริงก่อนส่งคำสั่งซื้อ`, tone: "positive" }); if (r > 25) rows.push({ title: `${item.ticker} กำไรเริ่มยืด`, text: `ผลตอบแทน ${item.pl}. ระวังไล่ราคาและทบทวน target weight`, tone: "caution" }); if (r < -5) rows.push({ title: `${item.ticker} ต้องเฝ้า drawdown`, text: `ผลตอบแทน ${item.pl}. ทบทวน thesis และ allocation gap`, tone: "caution" }); }); document.querySelectorAll(".alert-dot").forEach(button => button.dataset.count = String(Math.min(rows.length, 9))); setHtml("alertsList", rows.slice(0, 5).map(row => `<div class="alert-row"><div><strong>${row.title}</strong><p>${row.text}</p></div><span class="badge ${row.tone}">${row.tone}</span></div>`).join("") || `<div class="empty">ตอนนี้ไม่มี alert สำคัญจากข้อมูลล่าสุด</div>`); }
